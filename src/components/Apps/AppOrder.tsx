...
export const AppOrder: React.FC<AppOrderProps> = ({ service, onBack, onSave, onPreview }) => {
  ...
  console.log("AppOrder received service:", service);
  console.log("service.chapters:", service?.chapters);
  console.log("service.chapters?.length:", service?.chapters?.length);

  useEffect(() => {
    console.log("useEffect triggered with service:", service);
    ...
    console.log('Processing chapters:', service.chapters);
    ...
    console.log('Generated orderData:', data);
    setOrderData(data);
    setIsLoading(false);
  }, [service]);
    ...
    data.forEach((chapter, chapterIndex) => {
      chapter.data.forEach((field, fieldIndex) => {
        ...
        if (originalField) {
          ...
          const fileIndex = fieldIndex - (service.chapters[chapterIndex]?.data?.length || 0);
          const originalFile = service.chapters[chapterIndex]?.files?.[fileIndex];
          if (originalFile) {
  ...
  if (!service || !service.chapters || service.chapters.length === 0 || orderData.length === 0) {
    console.log("Condition for empty render triggered:", {
      hasService: !!service,
      hasChapters: !!service?.chapters,
      chaptersLength: service?.chapters?.length,
      orderDataLength: orderData.length
    });
    return <div>Нет данных для отображения формы</div>;
  }

  console.log("Rendering DataEditor with orderData:", orderData);
